...
GENES = {0: set(), 1: {'RhD'}, 2: {'B'}, 3: {'B', 'RhD'}, 4: {'A'},
         5: {'A', 'RhD'}, 6: {'A', 'B'}, 7: {'A', 'B', 'RhD'}}
...
def _covert_to_gene(bt):
    """ Convert input to set og genes
        Args:
        bt (int | str | Bloodtype): red blood cell type
        Returns:
    ...
    if isinstance(bt, str):
        if bt in blood_type_text:
            return GENES[blood_type_text[bt].value]
        else:
            raise ValueError

    if isinstance(bt, Bloodtype):
        return GENES[bt.value]
...
def check_bt(donor, recipient):
    ...
    """
    return len(_covert_to_gene(donor)-_covert_to_gene(recipient)) == 0
